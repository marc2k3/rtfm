...
!!! note
	...
	remove it. If the lock is owned by `JavaScript Panel` then
	...
	function add_playlist_lock(playlistIndex, mask) {
		...
		if (playlistIndex >= 0 && playlistIndex < plman.PlaylistCount) {
			if (plman.IsPlaylistLocked(playlistIndex)) {
				if (plman.GetPlaylistLockName(playlistIndex) == 'foo_javascript_panel') {
...
!!! note
	...
	owned by `JavaScript Panel`. You can check
